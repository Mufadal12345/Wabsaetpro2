...
export const Quotes: React.FC = () => {
  const { quotes, loadingData } = useData();
    ...
    <div className="h-full flex flex-col relative animate-fade-in">
      ...
      <header className="pb-2 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm">كلمات تخلد في الذاكرة</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="bg-accent hover:bg-pink-600 text-white px-5 py-2 rounded-full text-sm font-bold shadow-lg transition transform active:scale-95 flex items-center gap-2"
        >
      ...
      <div className="flex-1 overflow-y-auto pt-0 custom-scrollbar">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
          {loadingData && displayQuotes.length === 0 ? (
            <>
              <StatsSkeleton />
              <StatsSkeleton />
              <StatsSkeleton />
      ...
      <ConfirmModal
        ...
        message="هل أنت متأكد من حذف هذه العبارة؟"
        confirmText="حذف"
      />
    </div>
  );
