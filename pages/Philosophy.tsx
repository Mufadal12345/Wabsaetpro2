          ...
          {loadingData && philosophyIdeas.length === 0 ? (
            ...
            philosophyIdeas.map((item) => (
              ...
              <div className="mt-4 flex justify-between items-center relative z-10">
                ...
                {(currentUser?.role === "admin" || currentUser?.role === "super_admin" || currentUser?.id === item.authorId) && !item.id.startsWith("static") && (
                  <button
          ...
          <div className="bg-white/5 p-6 rounded-[2.5rem] border border-white/10 shadow-2xl backdrop-blur-xl">
            <div className="flex items-center gap-4 mb-8 border-b border-white/5 pb-6">
              <div className="shrink-0 flex items-center">
                <UserAvatar user={{ id: currentUser?.id || '', name: currentUser?.name || 'U', photoURL: currentUser?.photoURL }} className="w-16 h-16" />
              </div>
              <div>
