...
import { useVisualFeedQuery } from "../src/features/ideas/queries/idea.queries";
...
export const Visuals: React.FC = () => {
  ...
  const visibleIdeas = useMemo(() => {
    ...
    // Only visuals (anything with a URL or specifically marked as image/video)
    const regex = /(https?:\/\/[^\s]+)/i;
    all = all.filter(idea => regex.test(idea.content || "") || idea.imageUrl || idea.type === 'video' || idea.type === 'image');
            ...
            components={{
              Footer: () => (
                <div className="h-24 w-full flex items-center justify-center">
                  ...
                  {visibleIdeas.length === 0 && !loadingMoreIdeas && (
                    <div className="text-center py-20 text-gray-400">
                      لا توجد محتويات مرئية في هذا القسم.
                    </div>
                  )}
                </div>
              )
