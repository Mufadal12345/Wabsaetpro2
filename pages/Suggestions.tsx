...
export const Suggestions: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTitle("");
    setContent("");
    showToast("جاري إرسال مقترحك...", "info");
    ...
    try {
      ...
      showToast("فشل إرسال المقترح", "error");
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast("تم حذف المقترح بنجاح", "success");
    } catch (e) {
      // Rollback
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto font-tajawal">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold gradient-text mb-2 font-amiri">الرسائل والمقترحات</h2>
        <p className="text-gray-400">
          تواصل مع الإدارة وشاركنا أفكارك
        </p>
      </div>

      <div className="flex gap-4 mb-8 border-b border-white/10 pb-4 justify-center">
        <button
          onClick={() => setActiveTab("suggestions")}
          className={`px-6 py-2 rounded-full font-bold transition-all ${activeTab === "suggestions" ? "bg-accent text-white" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}
        >
          مقترحاتي
        </button>
        <button
          onClick={() => setActiveTab("inbox")}
          className={`px-6 py-2 rounded-full font-bold transition-all relative ${activeTab === "inbox" ? "bg-accent text-white" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}
        >
          صندوق الوارد
      ...
      <ConfirmModal
        ...
        message="هل أنت متأكد من حذف هذا المقترح؟"
        confirmText="حذف"
      />
    </div>
  );
