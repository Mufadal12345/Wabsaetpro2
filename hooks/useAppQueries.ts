...
import { Idea, User, IdeaComment, SystemNotification, AdminGroupMessage, AuditLog, Suggestion, Message, Quote, Course, Bookmark, Follow, SectionItem, SocialRelation, Story } from "../types";
...
export const useNotifications = () => {
  ...
  return useQuery({
    queryKey: ["notifications", currentUser?.id, currentUser?.role],
    queryFn: async () => {
      if (!currentUser) return [];
      let q = query(collection(db, "notifications"), orderBy("createdAt", "desc"));
      if (currentUser.role !== 'admin' && currentUser.role !== 'super_admin') {
        q = query(collection(db, "notifications"), where("recipientId", "==", currentUser.id), orderBy("createdAt", "desc"));
      }
      const snap = await getDocs(q);
...
export const useAdminChats = () => {
  const { currentUser } = useAuth();
  return useQuery({
    queryKey: ["admin-chats", currentUser?.id, currentUser?.role],
    queryFn: async () => {
...
export const useSuggestions = () => {
  ...
  return useQuery<Suggestion[]>({
    queryKey: ["suggestions", currentUser?.id, currentUser?.role],
    queryFn: async () => {
      if (!currentUser) return [];
      
      let q = query(collection(db, "suggestions"), orderBy("createdAt", "desc"));
      if (currentUser.role !== 'admin' && currentUser.role !== 'super_admin') {
        q = query(collection(db, "suggestions"), where("authorId", "==", currentUser.id), orderBy("createdAt", "desc"));
      }
...
export const useMessages = () => {
  ...
  return useQuery<Message[]>({
    queryKey: ["messages", currentUser?.id, currentUser?.role],
    queryFn: async () => {
      if (!currentUser) return [];
      let q = query(collection(db, "messages"), orderBy("createdAt", "desc"));
      if (currentUser.role !== 'admin' && currentUser.role !== 'super_admin') {
        q = query(collection(db, "messages"), where("fromId", "==", currentUser.id), orderBy("createdAt", "desc"));
      }
      const snap = await getDocs(q);
...
export const useBookmarks = () => {
  ...
  return useQuery<Bookmark[]>({
    ...
    queryFn: async () => {
      if (!currentUser) return [];
      let q = query(collection(db, "bookmarks"), where("userId", "==", currentUser.id), orderBy("createdAt", "desc"));
      const snap = await getDocs(q);
...
export const useSocialRelation = (userId: string) => {
  return useQuery<SocialRelation | null>({
    queryKey: ["social", userId],
    queryFn: async () => {
      if (!userId) return null;
      const d = await getDoc(doc(db, "social", userId));
      if (!d.exists()) return { id: userId, userId, followers: [], following: [] } as SocialRelation;
      return { id: d.id, ...d.data() } as SocialRelation;
    },
    enabled: !!userId
  });
};
...
export const useMultipleUsers = (userIds: string[]) => {
  return useQuery<User[]>({
    queryKey: ["users", "multiple", userIds],
    queryFn: async () => {
      if (!userIds || userIds.length === 0) return [];
      // Firestore has a limit of 10 for 'in' queries, but we can do it manually or fetch in chunks
      // For simplicity and assuming modest counts for now:
      const users: User[] = [];
      const chunks = [];
      ...
      for (const chunk of chunks) {
        ...
        const snap = await getDocs(q);
        users.push(...snap.docs.map(d => ({ id: d.id, ...d.data() } as User)));
      }
      return users;
    },
